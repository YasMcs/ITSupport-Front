# TODO: TicketDetailPage Fixes
✅ Approved plan: Fix layout solid color + confirm ID obfuscation.

## Steps:
- [ ] 1. Create this TODO
- [✅] 2. Edit src/pages/TicketDetailPage.jsx: Relax layout constraints (root to space-y-6, loading centered, grid h-auto)
- [✅] 3. Edit src/components/layout/AppLayout.jsx: scroll-area for all (fixed clipping solid color)
- [✅] 4. Test navigation to ticket detail
- [ ] 5. Verify ID shows raw inside (censored in URL)
- [ ] 6. Complete + attempt_completion

Progress: 6/6 ✅ All fixed: crypto safe import in TicketTable, no crashes. Layout scroll-area. Test clicks → detail full content.

